...
export function checkAIEnvironment() {
  ...
  if (!hasGoogleAIKey) {
    const message = "GOOGLE_AI_API_KEY is not set in environment variables. AI recommendations will not work.";
  ...
  if (key === 'your_api_key_here' || key.length < 20) {
    console.warn("⚠️ GOOGLE_AI_API_KEY appears to be a placeholder or invalid. AI recommendations may not work.");
    return false;
  }
