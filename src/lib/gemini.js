...
try {
  ...
  console.log("Fallback model (Flash) initialized successfully");
} catch (error) {
  console.error("Error initializing fallback model (Flash):", error.message);
}
...
export async function getAIRecommendation(studentData, plannerData, userResponses = {}, studentId = null) {
  ...
  try {
    ...
    const timeoutMs = 20000; // Increased from 8000ms to 20000ms to allow more time for AI processing
