...
// Define the path for the actual university logo
const UNIVERSITY_LOGO_PATH = '/registrarbanner.png';
...
export const usePDFGenerator = ({
  ...
  const generatePDF = (): void => {
    ...
    // Add university logo
    try {
      // Try to use the actual university logo first
      const img = new Image();
      img.src = window.location.origin + UNIVERSITY_LOGO_PATH;
      ...
      img.onload = () => {
        ...
        // Place logo at the top left with more space for text
        doc.addImage(img.src, 'PNG', 15, 10, 60, 20);
        // Continue with the rest of the PDF generation after logo is loaded
        continueWithPdfGeneration();
      };
      
      img.onerror = () => {
        // If actual logo fails, fallback to the base64 encoded logo
        console.warn('Failed to load university logo, using fallback');
        doc.addImage(BU_LOGO, 'PNG', 15, 10, 25, 25);
        continueWithPdfGeneration();
      };
      
      // Define the function to continue PDF generation after logo handling
      const continueWithPdfGeneration = () => {
        // Add university name and report title - positioned below the logo
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(16);
        doc.setTextColor(colors.primary[0], colors.primary[1], colors.primary[2]);
        // Move text further down to avoid overlap
        doc.text('GPA Calculator Report', 15, 38);
