// src/app/api/pdf-disclaimer/route.ts
...
export async function GET(request: Request) {
    ...
    // --- Define the sheet name and range for the PDF disclaimer ---
    const sheetName = 'Configuration'; // Or AppSettings, whatever you named it
    const range = `${sheetName}!C1`;    // Cell C1 contains the PDF disclaimer text 
    // ---------------------------------------------------------

    try {
        const sheets = await getSheetsClient();
        console.log(`Fetching PDF disclaimer from Sheet ID: ${spreadsheetId}, Range: ${range}`);
        ...
        let disclaimerText = "PDF disclaimer text not found in configured sheet/cell."; // Default message

        if (rows && rows.length > 0 && rows[0].length > 0) {
            disclaimerText = rows[0][0]; // Get text from the first cell of the first row
            console.log("Successfully fetched PDF disclaimer text.");
        } else {
            console.warn(`No data found for PDF disclaimer in sheet '${sheetName}' at range '${range}'.`);
        }
        ...
        console.error("Error fetching PDF disclaimer:", error.message || error);
        // Provide more specific error messages if possible
        let errorMessage = 'Failed to fetch PDF disclaimer text';
