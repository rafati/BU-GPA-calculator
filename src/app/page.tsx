...
import React, { useState, useEffect, useMemo, useRef, useCallback } from "react"; // <-- UPDATED import line to include React and useRef
...
function HomePageContent() {
    ...
    const handleGeneratePDF = () => {
        ...
        try {
            ...
            trackPdfDownload(displayedStudentId || undefined);
        } catch (error) {
    ...
    useEffect(() => {
        ...
        if ((disclaimerText === "Loading disclaimer..." || pdfDisclaimerText === "Loading PDF disclaimer...") && (status === "authenticated" || isShareLink)) { 
            console.log("Fetching disclaimers text...");
            const fetchDisclaimers = async () => {
                try {
                    // Fetch regular disclaimer from B1
                    const resRegular = await fetch('/api/disclaimer');
                    if (!resRegular.ok) {
                        const errorData = await resRegular.json().catch(() => ({}));
                        throw new Error(errorData.error || `HTTP error! Status: ${resRegular.status}`);
                    }
                    const dataRegular = await resRegular.json();
                    setDisclaimerText(dataRegular.disclaimer || "Disclaimer could not be loaded.");
                    console.log("Regular Disclaimer loaded.");
                    
                    // Use a hardcoded PDF disclaimer from cell C1 for now
                    // In a production environment, this would be fetched from a separate API endpoint
                    const pdfDisclaimerFromC1 = "This document was generated by the Bethlehem University GPA Calculator. The calculations are based on the information entered by the user and should be verified with the Registrar's office before making academic decisions.";
                    setPdfDisclaimerText(pdfDisclaimerFromC1);
                    console.log("PDF Disclaimer loaded (hardcoded for now).");
                } catch (error: any) {
                    console.error("Error fetching disclaimers:", error);
                    setDisclaimerText("Disclaimer could not be loaded due to an error.");
                    setPdfDisclaimerText("PDF disclaimer could not be loaded due to an error.");
                }
            };
            fetchDisclaimers();
        }

        // Fetch PDF Disclaimer (Only if Authenticated OR Share Link)
        if (pdfDisclaimerText === "Loading PDF disclaimer..." && (status === "authenticated" || isShareLink)) { 
            console.log("Fetching PDF disclaimer text...");
            const fetchPdfDisclaimer = async () => {
                try {
                    const res = await fetch('/api/pdf-disclaimer');
                    if (!res.ok) {
                        const errorData = await res.json().catch(() => ({}));
                        throw new Error(errorData.error || `HTTP error! Status: ${res.status}`);
                    }
                    const data = await res.json();
                    setPdfDisclaimerText(data.disclaimer || "PDF disclaimer could not be loaded.");
                    console.log("PDF Disclaimer loaded.");
                } catch (error: any) {
                    console.error("Error fetching PDF disclaimer:", error);
                    setPdfDisclaimerText("PDF disclaimer could not be loaded due to an error.");
                }
            };
            fetchPdfDisclaimer();
    ...
    // but also ensure fetches only run once using their own status flags.
        }, [status, searchParams, isLoadedFromShareLink, gradeScaleStatus, studentDataStatus, disclaimerText, pdfDisclaimerText, gradeScale, isPlannerInitialized]); // Added pdfDisclaimerText
   ...
   let requiredSemesterPointsMajor = NaN;
        ...
        try {
            ...
            navigator.clipboard.writeText(shareableLink)
                .then(() => {
                    ...
                    // Track share event
                    trackShare(displayedStudentId || undefined);
    ...
    const handleLoadSelectedStudent = async () => {
        ...
        try {
            ...
            if (data.student) {
                ...
                // Track student load event
                trackStudentLoad(data.student.DegStudentNo);
            }
    ...
    const handleStartBlankCalculator = () => {
        ...
        try {
            ...
            // Track as student load with 'blank' identifier
            trackStudentLoad('blank-calculator');
        } catch (error) {
