        ...
        if ((disclaimerText === "Loading disclaimer..." || pdfDisclaimerText === "Loading PDF disclaimer...") && (status === "authenticated" || isShareLink)) { 
            ...
            const fetchDisclaimers = async () => {
                try {
                    ...
                    // Fetch PDF disclaimer from C1
                    const resPdf = await fetch('/api/pdf-disclaimer');
                    if (!resPdf.ok) {
                        const errorData = await resPdf.json().catch(() => ({}));
                        throw new Error(errorData.error || `HTTP error! Status: ${resPdf.status}`);
                    }
                    const dataPdf = await resPdf.json();
                    setPdfDisclaimerText(dataPdf.disclaimer || "PDF disclaimer could not be loaded.");
